...
import { getKpiWiseMembers } from '@/shared/chart/branch_data';
  ...
  methods:{
      loadData: function(){
        ...
        this.setBranchList();
          ...
          let kpi_name = this.$t('female');
          if(kpi == 2){
              kpi_name = this.$t('male');;
          }
          let params = {
            branch_list: this.branch_list.join(),
            period: this.filter_props['period'],
          };
        this.$http_service.get('dashboard-service/branch/get_member_info',{params})
        .then(res => {
            this.data_info = {};
            if(Object.keys(res.data && res.data.data.members).length > 0) {
                this.data_info = {
                    data:getKpiWiseMembers(res.data.data.members, this.branch_list, kpi, this.filter_props.branch_name),
                    text:kpi_name,
                    type:'column',
                    height:300,
                    y_text:this.$t("value"),
